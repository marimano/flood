    ...
    function(Backbone, SaveFileMessage, SetModelPositionMessage,
        ...
        return Backbone.Model.extend({
            ...
            createWorkspaceWithData: function (params) {
              ...
              var workspaceReady = function (ws) {
                app.trigger('computation-completed:event', params);
                ...
                if (params.guid) {
                  this.setAvailableCustomNodeDefinitions(params.guid);
                }
              }.bind(this);

              var isCustomNode = !!params.guid;
              if (isCustomNode) {
                workspaces = allWorkspaces.where({guid: params.guid});
              }
              else {
              ...
              if (workspaces.length) {
                ...
                if (isCustomNode) {
                  // replace current custom node workspace
                  // instead of adding new one to avoid a mess with dependencies
                  app.openWorkspace(currentWorkspace.get('_id'), workspaceReady, params);
                }
                else {
                  // add new home to keep current one in Mongo.
                  app.newWorkspace(workspaceReady, params);
                }
              }
              else if (isCustomNode) {
                workspaces = this.app.workspaceBrowser ? this.app.workspaceBrowser
                  .get('workspaces').where({guid: params.guid}) : [];

                if (workspaces.length) {
                  app.openWorkspace(workspaces[0].get('_id'), workspaceReady, params);
                }
                else {
                  app.newNodeWorkspace(workspaceReady, null, params);
                }
              }
              else {
                app.newWorkspace(workspaceReady, params);
